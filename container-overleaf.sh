...
# If 'set -E' is not supported (dash, some sh), fallback to 'set -euo pipefail'
if ! set -E 2>/dev/null; then
  set -euo pipefail
fi
USERNS_MODE="${USERNS_MODE:-auto}"

# Helper to append UserNS mode if needed (for s6-based containers, userns=none is often required)
append_userns_mode() {
  local file="$1"
  local userns_mode="$2"
  if [[ -n "$userns_mode" && "$userns_mode" != "auto" ]]; then
    printf 'UserNS=%s\n' "$userns_mode" >> "$file"
  fi
}

STACK_NAME="${STACK_NAME:-container-overleaf}"
...
EOF
  ...
  append_userns_mode "$file" "$USERNS_MODE"
...
reload_and_start_quadlet() {
  ...
  if podman logs "$APP_NAME" 2>&1 | grep -q 's6-svscan: fatal: unable to open .s6-svscan/lock: Permission denied'; then
    printf '\n[ERROR] Overleaf failed to start due to s6 user namespace permissions.\n' >&2
    printf '[HINT] Set USERNS_MODE=none and re-run this script.\n' >&2
    exit 111
  fi
}
